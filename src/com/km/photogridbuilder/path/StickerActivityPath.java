...
//            StickerViewPath, ImageObjectPath, TextObject

public class StickerActivityPath extends Activity
    implements android.view.View.OnClickListener, StickerViewPath.TapListener, StickerViewPath.ClickListener, EffectSelectListener
{
    class BackgroungTask extends AsyncTask
    {
        protected Integer doInBackground(Void avoid[])
        {
            ...
            while(i < j)
            {
                ...
                imageobjectpath = new ImageObjectPath(bitmap, resources);
                imageobjectpath.setUrl((String)demo.get(i));
                imageobjectpath.setClipping(false);
                imageobjectpath.setBorder(true);
                view.init(imageobjectpath);
        ...
        protected void onPostExecute(Integer integer)
        {
            ...
            {
                Toast.makeText(StickerActivityPath.this, "Unable to create collage", 0).show();
                finish();
                return;
            } else
            {
    ...
    public class SaveOutputAsynchTask extends AsyncTask
    {
        ...
        protected void onPostExecute(Void void1)
        {
            ...
            if(isSaved)
            {
                Toast.makeText(StickerActivityPath.this, "Collage saved. Can be viewed anytime from \"Your Creations\" and shared with friends.", 1).show();
            } else
            {
                Toast.makeText(StickerActivityPath.this, "Unable to save Collage. Please Check Disk Space.", 1).show();
            }
            if(display != null)
            {
                display.recycle();
        ...
        {
            super();
            filePath = (new StringBuilder(String.valueOf(Environment.getExternalStorageDirectory().toString()))).append(getString(0x7f060061)).append(StickerActivityPath.getCurrentImageName()).toString();
    ...
    private void askForEditConfirmation()
    {
        ...
        builder.setItems(getResources().getStringArray(0x7f0b0007), new android.content.DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialoginterface, int i)
            {
                ...
                if(i == 0)
                {
                    String s2 = ((ImageObjectPath)CurrentObject).getUrl();
                    mOutputFilePath = s2;
                    startFeather(s2);
                    return;
                }
                s = "";
                s1 = ((ImageObjectPath)CurrentObject).getUrl();
                file = new File((new File(s1)).getParent());
                if(file.exists()){
	                String as[] = file.list();
	                if(as.length > 0){
		                int j = 0;
                ...
                mOutputFilePath = s1;
                startFeather((new StringBuilder(String.valueOf(file.getAbsolutePath()))).append(File.separator).append(s).toString());
                return;
            }
        });
        builder.show();
    ...
    private void askForPasteConfirmation()
    {
        ...
        builder.setItems(getResources().getStringArray(0x7f0b0006), new android.content.DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialoginterface, int i)
            {
                ...
                mOutputFilePath = s1;
                startCutPhoto((new StringBuilder(String.valueOf(file.getAbsolutePath()))).append(File.separator).append(s).toString());
            }
        });
        builder.show();
    }
    ...
    private void lenghtyAddTask(CustomTouchPath.PointInfo pointinfo, Path path)
    {
        float af[][];
        Region region;
        RectF rectf;
        ...
        if(mType == 102)
        {
            af = Constant.getShapeAddForLayout(resourceID);
        } else
        {
            af = Constant.getIrrgularAddForLayout(resourceID);
        }
        region = new Region();
        rectf = new RectF();
        ...
        do
        {
            ...
            float f2 = af[i][2];
            float f3 = af[i][3];
            RectF rectf1 = new RectF(f - f2 / 2.0F, f1 - f3 / 2.0F, f + f2 / 2.0F, f1 + f3 / 2.0F);
            mapPoints(point, rectf1);
            if(region.contains((int)(f * mScaleX + view.gapRect.left), (int)(f1 * mScaleY + view.gapRect.top)))
            {
                view.addButton(i);
                return;
            }
    ...
    private void lenghtyTask(String s)
    {
        ...
        {
            af = Constant.getShapeAddForLayout(resourceID);
            af1 = Constant.getShapePathForLayout(resourceID);
        } else
        {
            af = Constant.getIrrgularAddForLayout(resourceID);
            af1 = Constant.getIrrgularPathForLayout(resourceID);
        }
        path = new Path();
        flag = true;
        f = af[-1 + choice][0];
        ...
        f2 = af[-1 + choice][2];
        f3 = af[-1 + choice][3];
        rectf = new RectF(f - f2 / 2.0F, f1 - f3 / 2.0F, f + f2 / 2.0F, f1 + f3 / 2.0F);
        bitmap = BitmapUtil.getBitmap(this, s, point.x / 2, point.y / 2);
        rectf1 = mapPoints(point, rectf);
        af2 = af1[-1 + choice];
        i = 0;
        do
        {
            if(i >= af2.length)
            {
                ...
                imageobjectpath.setUrl(s);
                imageobjectpath.setBorder(false);
                view.init(imageobjectpath);
                view.loadImages(getBaseContext(), rectf1, path);
            ...
            if(flag)
            {
                ...
                path.moveTo(af2[i][0] * mScaleX + view.gapRect.left, af2[i][1] * mScaleY + view.gapRect.top);
            } else
            {
                path.lineTo(af2[i][0] * mScaleX + view.gapRect.left, af2[i][1] * mScaleY + view.gapRect.top);
            }
            i++;
        } while(true);
    }

    private void lenghtyTaskAfteEffectApplied(String s, Path path, Object obj)
    {
        float af[][];
        float af1[][][];
        float f;
        ...
        try{
	        ...
	        rectf = new RectF(f - f2 / 2.0F, f1 - f3 / 2.0F, f + f2 / 2.0F, f1 + f3 / 2.0F);
	        bitmap = BitmapUtil.getBitmap(this, s, point.x / 2, point.y / 2);
	        mapPoints(point, rectf);
	        float[][] _tmp = af1[-1 + choice];
	        region = new Region();
	        rectf1 = new RectF();
	        path.computeBounds(rectf1, true);
	        region.setPath(path, new Region((int)rectf1.left, (int)rectf1.top, (int)rectf1.right, (int)rectf1.bottom));
	        i = 0;
	        do
	        {
	            if(i >= af.length)
	            ...
	            if(region.contains((int)tempPointInfo.getX(), (int)tempPointInfo.getY()))
	            {
	                getResources();
	                ((ImageObjectPath)obj).setBitmap(bitmap);
	                return;
	            }
	            i++;
	        } while(true);
    ...
    public Bitmap getStickers()
    {
        ...
        while(i < j){
            ...
            if(!(obj2 instanceof ImageObjectPath) && (obj2 instanceof TextObject))
            {
                arraylist1.add(obj2);
            }
            i++;
        ...
        while(iterator.hasNext()){
            ...
            boolean flag = obj instanceof ImageObjectPath;
            int k1 = 0;
            boolean flag1 = false;
            boolean flag2 = false;
            float f4 = 0.0F;
            ...
            if(flag)
            {
                ...
                paint = imageobjectpath1.getPaint();
                bitmap2 = imageobjectpath1.getBitmap();
                flag2 = imageobjectpath1.isClipping();
                rectf = imageobjectpath1.getOriginalRect();
                float1 = Float.valueOf(imageobjectpath1.getAngle());
                f6 = imageobjectpath1.getMinX();
                f4 = imageobjectpath1.getMaxX();
                f5 = imageobjectpath1.getMaxY();
                f7 = imageobjectpath1.getMinY();
                flag1 = imageobjectpath1.isBorder();
                k1 = imageobjectpath1.borderSize;
            }
            if(!(obj instanceof TextObject))
            {
                if(obj instanceof ImageObjectPath)
                {
                    ...
                    bitmap2 = imageobjectpath.getBitmap();
                    float1 = Float.valueOf(imageobjectpath.getAngle());
                    f6 = imageobjectpath.getMinX();
                    f4 = imageobjectpath.getMaxX();
                    f5 = imageobjectpath.getMaxY();
                    f7 = imageobjectpath.getMinY();
                    flag1 = imageobjectpath.isBorder();
                }
    ...
    protected void onActivityResult(int i, int j, Intent intent)
    {
        ...
        case 0:
            ...
            backgroungtask3.execute(new Void[0]);
            return;
        case 10:
        	int i1;
            int j1;
            ...
            while(true){
    	        ...
    	        {
    	            ((ImageObjectPath)obj1).setBitmap(bitmap1);
    	            ((ImageObjectPath)obj1).setClipping(false);
    	            ((ImageObjectPath)obj1).setBorder(false);
    	            ((ImageObjectPath)obj1).setFirstLoad(false);
    	            ((ImageObjectPath)obj1).load(getResources());
    	            ((ImageObjectPath)obj1).setUrl(mOutputFilePath);
    	        }
            }
        case 100:
        	if(!(j != -1 || intent == null)){
    	        if(!isFixedCollageClicked){
    	            ...
    	            if(demo == null){
    	                ...
    	                if(s == null){
    	                    ...
    	                    if(demo != null)
    	                    {
    	                        ...
    	                        backgroungtask2.execute(new Void[0]);
    	                    }
    	                }else{
    	        	        demo.add(s);
    	        	        if(demo != null)
    	        	        {
    	        	            ...
    	        	            backgroungtask1.execute(new Void[0]);
    	        	        }
    	                }
    	            }else{
    	    		    BackgroungTask backgroungtask = new BackgroungTask();
    	    		    backgroungtask.execute(new Void[0]);
    	            }
    	        }else{
    		        ...
    		        if(s1 != null){
    			        AsyncTask asynctask = new AsyncTask() {
    			            ...
    			            protected void onPreExecute()
    			            {
    			                pdDialog = new ProgressDialog(StickerActivityPath.this);
        ...
        try{
	        if(!isFreeFlow)
	        {
	            Path path = ((ImageObjectPath)CurrentObject).getPath();
	            lenghtyTaskAfteEffectApplied(mOutputFilePath, path, CurrentObject);
	            view.invalidate();
	        ...
	        while(iterator.hasNext()) 
	        {
	            ...
	            {
	                ((ImageObjectPath)obj).setBitmap(bitmap);
	                ((ImageObjectPath)obj).setClipping(false);
	                ((ImageObjectPath)obj).setBorder(false);
	                ((ImageObjectPath)obj).setFirstLoad(false);
	                ((ImageObjectPath)obj).load(getResources());
	                ((ImageObjectPath)obj).setUrl(mOutputFilePath);
	            }
	        }
        }catch(Exception e){}
    }
    ...
    public void onClick(View view1)
    {
        ...
        case 2131296464:
            ...
            view.loadImages(this, rectf, null);
            view.invalidate();
    ...
    public void onCreate(Bundle bundle)
    {
        ...
        view = (StickerViewPath)findViewById(0x7f090032);
        view.setType(mType);
        ...
        while(true){
	        if(j >= as1.length)
	        {
	            ...
	            tracker.setScreenName("StickerActivityPath");
    ...
    public void onDoubleTapListener(final Object img, final CustomTouchPath.PointInfo touchPoint, int i)
    {
        if(img != null)
        {
            android.app.AlertDialog.Builder builder = new android.app.AlertDialog.Builder(this);
                ...
                builder.setItems(as, new android.content.DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialoginterface, int j)
                    {
                        if(j != 0){
                            ...
                            {
                                view.delete(img);
                                if(!isFreeFlow && (img instanceof ImageObjectPath))
                                {
                            ...
                            {
                                CurrentObject = img;
                                File file = new File(((ImageObjectPath)CurrentObject).getUrl());
                                if(file.getParent().contains("CutPaste_") && file.getParentFile().exists())
                                {
                                    ...
                                    {
                                        String s1 = ((ImageObjectPath)img).getUrl();
                                        startCutPhoto(s1);
                                        return;
                                    }
                                }
                                ...
                                File file2 = new File(((ImageObjectPath)CurrentObject).getUrl());
                                if(file2.getParent().contains("Edit_") && file2.getParentFile().exists() && file2.getParentFile().list().length > 1)
                                {
                                    File file3 = checkforOriginalandSave(((ImageObjectPath)CurrentObject).getUrl());
                                    if(file3 != null)
                                    {
                                ...
                                String s = ((ImageObjectPath)img).getUrl();
                                startCutPhoto(s);
                            }
                            return;
                        }
                        ...
                        CurrentObject = img;
                        file5 = new File(((ImageObjectPath)CurrentObject).getUrl());
                        if(!file5.getParent().contains("Edit_") && !file5.getParent().contains("CutPaste_") || !file5.getParentFile().exists()){
                            ...
                            File file7 = new File(((ImageObjectPath)CurrentObject).getUrl());
                            ...
                            {
                                File file8 = checkforOriginalandSave(((ImageObjectPath)CurrentObject).getUrl());
                                if(file8 != null)
                                {
                            ...
                            String s2 = ((ImageObjectPath)img).getUrl();
                            startFeather(s2);
                            return;
                        }
                        if(file5.getParentFile().list().length <= 0){
                        	String s3 = ((ImageObjectPath)img).getUrl();
                            startFeather(s3);
                            return;
                        }
